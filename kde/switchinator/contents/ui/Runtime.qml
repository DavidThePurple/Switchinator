...
QtObject {
    ...
    required property var nativeEffect
    readonly property var configuration: nativeEffect.configuration
    property Component delegate
    function viewForScreen(output) { return nativeEffect.viewForScreen(output); }
    function activateView(view) { nativeEffect.activateView(view); }
    ...
    function cancel() {
        console.log("Switchinator: cancel requested");
    ...
    function finishSelection() {
        console.log("Switchinator: completing selection");
    ...
    onVisibleChanged: {
        if (nativeEffect) nativeEffect.visible=visible;
        if (!visible) {captureQueue=[];captureBusy=false;snapshots={};}
    }
    property QtObject themePalette: SystemPalette { id: palette; colorGroup: SystemPalette.Active }
    ...
    property QtObject forwardShortcut: KWin.ShortcutHandler {
        name: "SwitchinatorForward";text: "Switchinator: next window";sequence: "Meta+Tab"
        onActivated: effect.begin(false)
    }
    property QtObject backwardShortcut: KWin.ShortcutHandler {
        name: "SwitchinatorBackward";text: "Switchinator: previous window";sequence: "Meta+Shift+Tab"
        onActivated: effect.begin(true)
    }
    property QtObject captureTimer: Timer { interval: 150;repeat: true;running: effect.visible && effect.captureQueue.length>0;onTriggered: effect.captureNext() }
    property QtObject refreshTimer: Timer {
    ...
    property QtObject rotationTimer: Timer {
        id: rotation
    ...
    property QtObject returnTransition: NumberAnimation {
    ...
    property QtObject activationTimer: Timer {
    ...
    property QtObject safetyTimer: Timer {
    ...
    property QtObject finishTimer: Timer {
    ...
    property QtObject windowEvents: Connections {
    ...
    delegate: Item {
        ...
        function restoreInput() {
            ...
            console.log("Switchinator: input view ready",effect.host,scene.activeFocus);
        }
